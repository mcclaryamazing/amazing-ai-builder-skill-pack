...
Before launch, rerun the Customer-Facing Copy Gate from SKILL.md and the rendered sweep in final-wireframe-review.md on the implemented page. Seller notes and non-image verification placeholders must not reach customers; retain seller-supplied or approved buyer disclosures and record missing evidence as separate launch blockers.
...
## Page Release Versus Theme Replacement

The default release uploads approved page files to the existing live theme and publishes the Page. It does not publish the entire shared preview theme. If the user explicitly requests replacing the live theme, review the entire candidate and unrelated changes, record the previous live theme for rollback, verify the installed CLI's `theme publish` usage, publish only the approved theme ID, and re-list themes to verify the new live ID. Admin API scopes alone do not prove theme-publishing access; verify the Theme Access CLI path. Verify the resulting storefront and purchase paths after either release type.
