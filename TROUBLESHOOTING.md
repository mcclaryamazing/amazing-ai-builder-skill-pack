...
1. Confirm the expected `SKILL.md` file exists:
   - `skills/amazing-shopify-chatbot-builder/SKILL.md`
   - `skills/amazing-landing-page-builder/SKILL.md`
   - `skills/amazing-shopify-site-builder/SKILL.md`
...
```text
Use the amazing-shopify-chatbot-builder skill. Do not install or enable any storefront widget yet. First build the protected admin dashboard/control plane with health/readiness status, chatbot mode, fast disable path, Shopify sync, source counts, source browser, active/published source filters, embedded dashboard test chat, risky-question tests, support routing, and no exposed secrets.
```
...
```text
Use the amazing-landing-page-builder skill. Do not touch Shopify theme files yet. First create the Shopify-ready DTC design package with archetype, big idea, copy strategy, section sequence, offer/proof logic, CTA path, claim notes, metadata direction, and exact image-slot plan.
```
...
```text
Use the amazing-landing-page-builder skill in full-design process. Ask only for the missing facts that materially change the page archetype, CTA path, offer truth, claim/compliance boundaries, product selection logic, Shopify build path, or launch safety. Otherwise infer conservative defaults, label assumptions, and use verification placeholders.
```
...
```text
Use the amazing-landing-page-builder skill. Do not invent review counts, discounts, guarantees, certifications, testimonials, shipping terms, urgency, clinical claims, or comparison claims. Use verification placeholders or soften the copy until I provide proof.
```
...
```text
Use the amazing-landing-page-builder skill. Build the landing page in real Shopify theme files: section, CSS asset, optional JS asset, and JSON page template. Use Shopify Files for merchant-facing images and exact placeholders for missing images.
```
...
```text
Use the amazing-shopify-site-builder skill. Do not treat this as one campaign page. First create the Shopify site blueprint with site map, navigation, homepage plan, PDP template plan, collection template plan, content-page plan, shared theme system, reusable sections, data plan, metadata direction, image-slot plan, QA plan, and rollback path.
```
...
```text
Use the amazing-shopify-site-builder skill. Before broad theme work, verify or guide setup for both access rails: Theme Access plus Shopify CLI for theme files, and a Shopify Dev Dashboard app plus Admin GraphQL client credentials for products, collections, pages, files, menus, redirects, discounts, publications, metafields, and metaobjects. Reuse existing setup when safe and do not ask me to paste secrets into chat.
```
...
```text
Use the amazing-shopify-site-builder skill. Preserve Shopify-native product forms, variant selection, price/availability display, collection filters, sorting, pagination, product-card behavior, cart updates, and checkout links unless I explicitly approve a change. Re-test homepage, one product page, one collection page, one standard page, and cart/checkout path.
```
...
```text
Use the amazing-shopify-chatbot-builder skill. Review the dashboard test chat for unsupported claims about price, discounts, inventory, shipping, returns, warranty, policy exceptions, safety, and order status. Fix the prompt, retrieval, source filters, or refusal behavior before any theme install.
```
...
```text
Use the amazing-landing-page-builder skill. Run the landing-page QA checklist: verify the approved DTC spec, CSS and JS asset loading, Shopify Files image references, exact placeholders, desktop and mobile layout, CTA wiring, metadata, theme chrome, sticky CTA behavior, and rollback path.
```
