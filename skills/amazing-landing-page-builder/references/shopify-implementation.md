...
1. Read the full DTC design spec and internally reviewed placeholder-only wireframe. Before selecting, generating or inserting any final image, verify wireframe approval or an explicit end-to-end waiver of the pause. The artifact and internal review cannot be waived.
...
7. Use exact placeholders for every image until the wireframe has passed internal review and the image approval gate.
...
Before final-image reference selection or production, read `product-image-production.md`. It routes to the current available `amazing-product-photography` skill or a self-contained distilled fallback. Complete fidelity and aesthetic review/remediation before inserting accepted images, then verify their rendered page crops.
...
In the first wireframe, render visible placeholders for every image, even when assets exist. After the image approval gate, retain placeholders where final images remain unavailable. Preserve the final desktop/mobile position, aspect ratio, size behavior, crop behavior, alignment, and spacing.
